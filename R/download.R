...
#'
#' @param domain Character. The domain to download data from. Options are "westcoms2" or "etive28". Default is "westcoms2".
#' @param year Numeric. The year of the data to download. Default is 2024.
#' @param out_dir Character. The output directory where the downloaded files will be saved. Default is "~/".
#' @param cores Numeric. The number of cores to use for parallel processing. Default is 4.
#' @param overwrite Logical. Whether to overwrite existing files. Default is FALSE.
#' @param start_ymd Date. The start date for the data to download. Default is NULL, which sets it to January 1st of the specified year.
#' @param end_ymd Date. The end date for the data to download. Default is NULL, which sets it to December 31st of the specified year.
#'
...
download_from_thredds <- function(domain="westcoms2",
  ...
  out_dir <- glue("{out_dir}/{domain_name}/netcdf_{year}/")
